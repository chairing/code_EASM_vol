...
for f=1:ncfilenumber
    st_clim=ncread([path ncfile(f).name],'wap');
...
%     w_year1_clim(:,:,:,f)=squeeze(Xevents(:,:,before+2,:));
%          w_year1(:,:,:,f)=squeeze(Xeventss(:,:,before+2,:));
    Xeventss_all(:,:,:,:,f)=Xeventss;
    Xevents_all(:,:,:,:,f)=Xevents;
end

% save w_year1_20mjjas_clim.mat w_year1_clim;
% save w_year1_20mjjas.mat w_year1;
save w_wj_20mjjas_clim.mat Xevents_all;
save w_wj_20mjjas.mat Xeventss_all;